...
let zqlookStartbody18= $.isNode() ? (process.env.zqlookStartbody18 ? process.env.zqlookStartbody18 : "") : ($.getdata('zqlookStartbody18') ? $.getdata('zqlookStartbody18') : "")
let zqlookStartbody18Arr = []
let zqlookStartbody18s = ""
...
if (zqlookStartbody18) {
    if (zqlookStartbody18.indexOf("&") == -1) {
        zqlookStartbody18Arr.push(zqlookStartbody18)
    } else if (zqlookStartbody18.indexOf("&") > -1) {
        zqlookStartbody18s = zqlookStartbody18.split("&")
    } else if (process.env.zqlookStartbody18 && process.env.zqlookStartbody18.indexOf('&') > -1) {
        zqlookStartbody18Arr = process.env.zqlookStartbody18.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody18 = fs.readFileSync("zqlookStartbody18.txt", "utf8");
    if (zqlookStartbody18 !== `undefined`) {
        zqlookStartbody18s = zqlookStartbody18.split("\n");
    } else {
...
Object.keys(zqlookStartbody18s).forEach((item) => {
    if (zqlookStartbody18s[item] && !zqlookStartbody18s[item].startsWith("#")) {
        zqlookStartbody18Arr.push(zqlookStartbody18s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody18()
     $.done()
 }else{
    console.log(`共${zqlookStartbody18Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody18Arr.length; k++) {

                zqlookStartbody181 = zqlookStartbody18Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody18() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody18) {
            if (zqlookStartbody18.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody18.indexOf(bodyVal) == -1) {
                zqlookStartbody18s = zqlookStartbody18 + "&" + bodyVal;
                $.setdata(zqlookStartbody18s, 'zqlookStartbody18');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody18s: ${bodyVal}`);
                bodys = zqlookStartbody18s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody18');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody18s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody181,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody18').replace(zqlookStartbody181 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody18');
