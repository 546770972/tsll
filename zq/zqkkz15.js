...
let zqlookStartbody15= $.isNode() ? (process.env.zqlookStartbody15 ? process.env.zqlookStartbody15 : "") : ($.getdata('zqlookStartbody15') ? $.getdata('zqlookStartbody15') : "")
let zqlookStartbody15Arr = []
let zqlookStartbody15s = ""
...
if (zqlookStartbody15) {
    if (zqlookStartbody15.indexOf("&") == -1) {
        zqlookStartbody15Arr.push(zqlookStartbody15)
    } else if (zqlookStartbody15.indexOf("&") > -1) {
        zqlookStartbody15s = zqlookStartbody15.split("&")
    } else if (process.env.zqlookStartbody15 && process.env.zqlookStartbody15.indexOf('&') > -1) {
        zqlookStartbody15Arr = process.env.zqlookStartbody15.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody15 = fs.readFileSync("zqlookStartbody15.txt", "utf8");
    if (zqlookStartbody15 !== `undefined`) {
        zqlookStartbody15s = zqlookStartbody15.split("\n");
    } else {
...
Object.keys(zqlookStartbody15s).forEach((item) => {
    if (zqlookStartbody15s[item] && !zqlookStartbody15s[item].startsWith("#")) {
        zqlookStartbody15Arr.push(zqlookStartbody15s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody15()
     $.done()
 }else{
    console.log(`共${zqlookStartbody15Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody15Arr.length; k++) {

                zqlookStartbody151 = zqlookStartbody15Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody15() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody15) {
            if (zqlookStartbody15.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody15.indexOf(bodyVal) == -1) {
                zqlookStartbody15s = zqlookStartbody15 + "&" + bodyVal;
                $.setdata(zqlookStartbody15s, 'zqlookStartbody15');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody15s: ${bodyVal}`);
                bodys = zqlookStartbody15s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody15');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody15s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody151,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody15').replace(zqlookStartbody151 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody15');
