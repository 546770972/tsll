...
let zqlookStartbody14= $.isNode() ? (process.env.zqlookStartbody14 ? process.env.zqlookStartbody14 : "") : ($.getdata('zqlookStartbody14') ? $.getdata('zqlookStartbody14') : "")
let zqlookStartbody14Arr = []
let zqlookStartbody14s = ""
...
if (zqlookStartbody14) {
    if (zqlookStartbody14.indexOf("&") == -1) {
        zqlookStartbody14Arr.push(zqlookStartbody14)
    } else if (zqlookStartbody14.indexOf("&") > -1) {
        zqlookStartbody14s = zqlookStartbody14.split("&")
    } else if (process.env.zqlookStartbody14 && process.env.zqlookStartbody14.indexOf('&') > -1) {
        zqlookStartbody14Arr = process.env.zqlookStartbody14.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody14 = fs.readFileSync("zqlookStartbody14.txt", "utf8");
    if (zqlookStartbody14 !== `undefined`) {
        zqlookStartbody14s = zqlookStartbody14.split("\n");
    } else {
...
Object.keys(zqlookStartbody14s).forEach((item) => {
    if (zqlookStartbody14s[item] && !zqlookStartbody14s[item].startsWith("#")) {
        zqlookStartbody14Arr.push(zqlookStartbody14s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody14()
     $.done()
 }else{
    console.log(`共${zqlookStartbody14Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody14Arr.length; k++) {

                zqlookStartbody141 = zqlookStartbody14Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody14() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody14) {
            if (zqlookStartbody14.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody14.indexOf(bodyVal) == -1) {
                zqlookStartbody14s = zqlookStartbody14 + "&" + bodyVal;
                $.setdata(zqlookStartbody14s, 'zqlookStartbody14');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody14s: ${bodyVal}`);
                bodys = zqlookStartbody14s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody14');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody14s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody141,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody14').replace(zqlookStartbody141 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody14');
