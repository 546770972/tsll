...
let zqlookStartbody12= $.isNode() ? (process.env.zqlookStartbody12 ? process.env.zqlookStartbody12 : "") : ($.getdata('zqlookStartbody12') ? $.getdata('zqlookStartbody12') : "")
let zqlookStartbody12Arr = []
let zqlookStartbody12s = ""
...
if (zqlookStartbody12) {
    if (zqlookStartbody12.indexOf("&") == -1) {
        zqlookStartbody12Arr.push(zqlookStartbody12)
    } else if (zqlookStartbody12.indexOf("&") > -1) {
        zqlookStartbody12s = zqlookStartbody12.split("&")
    } else if (process.env.zqlookStartbody12 && process.env.zqlookStartbody12.indexOf('&') > -1) {
        zqlookStartbody12Arr = process.env.zqlookStartbody12.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody12 = fs.readFileSync("zqlookStartbody12.txt", "utf8");
    if (zqlookStartbody12 !== `undefined`) {
        zqlookStartbody12s = zqlookStartbody12.split("\n");
    } else {
...
Object.keys(zqlookStartbody12s).forEach((item) => {
    if (zqlookStartbody12s[item] && !zqlookStartbody12s[item].startsWith("#")) {
        zqlookStartbody12Arr.push(zqlookStartbody12s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody12()
     $.done()
 }else{
    console.log(`共${zqlookStartbody12Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody12Arr.length; k++) {

                zqlookStartbody121 = zqlookStartbody12Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody12() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody12) {
            if (zqlookStartbody12.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody12.indexOf(bodyVal) == -1) {
                zqlookStartbody12s = zqlookStartbody12 + "&" + bodyVal;
                $.setdata(zqlookStartbody12s, 'zqlookStartbody12');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody12s: ${bodyVal}`);
                bodys = zqlookStartbody12s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody12');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody12s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody121,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody12').replace(zqlookStartbody121 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody12');
