...
let zqlookStartbody11= $.isNode() ? (process.env.zqlookStartbody11 ? process.env.zqlookStartbody11 : "") : ($.getdata('zqlookStartbody11') ? $.getdata('zqlookStartbody11') : "")
let zqlookStartbody11Arr = []
let zqlookStartbody11s = ""
...
if (zqlookStartbody11) {
    if (zqlookStartbody11.indexOf("&") == -1) {
        zqlookStartbody11Arr.push(zqlookStartbody11)
    } else if (zqlookStartbody11.indexOf("&") > -1) {
        zqlookStartbody11s = zqlookStartbody11.split("&")
    } else if (process.env.zqlookStartbody11 && process.env.zqlookStartbody11.indexOf('&') > -1) {
        zqlookStartbody11Arr = process.env.zqlookStartbody11.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody11 = fs.readFileSync("zqlookStartbody11.txt", "utf8");
    if (zqlookStartbody11 !== `undefined`) {
        zqlookStartbody11s = zqlookStartbody11.split("\n");
    } else {
...
Object.keys(zqlookStartbody11s).forEach((item) => {
    if (zqlookStartbody11s[item] && !zqlookStartbody11s[item].startsWith("#")) {
        zqlookStartbody11Arr.push(zqlookStartbody11s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody11()
     $.done()
 }else{
    console.log(`共${zqlookStartbody11Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody11Arr.length; k++) {

                zqlookStartbody111 = zqlookStartbody11Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody11() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody11) {
            if (zqlookStartbody11.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody11.indexOf(bodyVal) == -1) {
                zqlookStartbody11s = zqlookStartbody11 + "&" + bodyVal;
                $.setdata(zqlookStartbody11s, 'zqlookStartbody11');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody11s: ${bodyVal}`);
                bodys = zqlookStartbody11s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody11');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody11s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody111,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody11').replace(zqlookStartbody111 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody11');
