...
let zqlookStartbody2= $.isNode() ? (process.env.zqlookStartbody2 ? process.env.zqlookStartbody2 : "") : ($.getdata('zqlookStartbody2') ? $.getdata('zqlookStartbody2') : "")
let zqlookStartbody2Arr = []
let zqlookStartbody2s = ""
...
if (zqlookStartbody2) {
    if (zqlookStartbody2.indexOf("&") == -1) {
        zqlookStartbody2Arr.push(zqlookStartbody2)
    } else if (zqlookStartbody2.indexOf("&") > -1) {
        zqlookStartbody2s = zqlookStartbody2.split("&")
    } else if (process.env.zqlookStartbody2 && process.env.zqlookStartbody2.indexOf('&') > -1) {
        zqlookStartbody2Arr = process.env.zqlookStartbody2.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody2 = fs.readFileSync("zqlookStartbody2.txt", "utf8");
    if (zqlookStartbody2 !== `undefined`) {
        zqlookStartbody2s = zqlookStartbody2.split("\n");
    } else {
...
Object.keys(zqlookStartbody2s).forEach((item) => {
    if (zqlookStartbody2s[item] && !zqlookStartbody2s[item].startsWith("#")) {
        zqlookStartbody2Arr.push(zqlookStartbody2s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody2()
     $.done()
 }else{
    console.log(`共${zqlookStartbody2Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody2Arr.length; k++) {

                zqlookStartbody21 = zqlookStartbody2Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody2() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody2) {
            if (zqlookStartbody2.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody2.indexOf(bodyVal) == -1) {
                zqlookStartbody2s = zqlookStartbody2 + "&" + bodyVal;
                $.setdata(zqlookStartbody2s, 'zqlookStartbody2');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody2s: ${bodyVal}`);
                bodys = zqlookStartbody2s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody2');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody2s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody21,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody2').replace(zqlookStartbody21 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody2');
