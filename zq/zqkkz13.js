...
let zqlookStartbody13= $.isNode() ? (process.env.zqlookStartbody13 ? process.env.zqlookStartbody13 : "") : ($.getdata('zqlookStartbody13') ? $.getdata('zqlookStartbody13') : "")
let zqlookStartbody13Arr = []
let zqlookStartbody13s = ""
...
if (zqlookStartbody13) {
    if (zqlookStartbody13.indexOf("&") == -1) {
        zqlookStartbody13Arr.push(zqlookStartbody13)
    } else if (zqlookStartbody13.indexOf("&") > -1) {
        zqlookStartbody13s = zqlookStartbody13.split("&")
    } else if (process.env.zqlookStartbody13 && process.env.zqlookStartbody13.indexOf('&') > -1) {
        zqlookStartbody13Arr = process.env.zqlookStartbody13.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody13 = fs.readFileSync("zqlookStartbody13.txt", "utf8");
    if (zqlookStartbody13 !== `undefined`) {
        zqlookStartbody13s = zqlookStartbody13.split("\n");
    } else {
...
Object.keys(zqlookStartbody13s).forEach((item) => {
    if (zqlookStartbody13s[item] && !zqlookStartbody13s[item].startsWith("#")) {
        zqlookStartbody13Arr.push(zqlookStartbody13s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody13()
     $.done()
 }else{
    console.log(`共${zqlookStartbody13Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody13Arr.length; k++) {

                zqlookStartbody131 = zqlookStartbody13Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody13() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody13) {
            if (zqlookStartbody13.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody13.indexOf(bodyVal) == -1) {
                zqlookStartbody13s = zqlookStartbody13 + "&" + bodyVal;
                $.setdata(zqlookStartbody13s, 'zqlookStartbody13');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody13s: ${bodyVal}`);
                bodys = zqlookStartbody13s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody13');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody13s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody131,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody13').replace(zqlookStartbody131 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody13');
