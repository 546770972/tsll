...
let zqlookStartbody4= $.isNode() ? (process.env.zqlookStartbody4 ? process.env.zqlookStartbody4 : "") : ($.getdata('zqlookStartbody4') ? $.getdata('zqlookStartbody4') : "")
let zqlookStartbody4Arr = []
let zqlookStartbody4s = ""
...
if (zqlookStartbody4) {
    if (zqlookStartbody4.indexOf("&") == -1) {
        zqlookStartbody4Arr.push(zqlookStartbody4)
    } else if (zqlookStartbody4.indexOf("&") > -1) {
        zqlookStartbody4s = zqlookStartbody4.split("&")
    } else if (process.env.zqlookStartbody4 && process.env.zqlookStartbody4.indexOf('&') > -1) {
        zqlookStartbody4Arr = process.env.zqlookStartbody4.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody4 = fs.readFileSync("zqlookStartbody4.txt", "utf8");
    if (zqlookStartbody4 !== `undefined`) {
        zqlookStartbody4s = zqlookStartbody4.split("\n");
    } else {
...
Object.keys(zqlookStartbody4s).forEach((item) => {
    if (zqlookStartbody4s[item] && !zqlookStartbody4s[item].startsWith("#")) {
        zqlookStartbody4Arr.push(zqlookStartbody4s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody4()
     $.done()
 }else{
    console.log(`共${zqlookStartbody4Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody4Arr.length; k++) {

                zqlookStartbody41 = zqlookStartbody4Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody4() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody4) {
            if (zqlookStartbody4.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody4.indexOf(bodyVal) == -1) {
                zqlookStartbody4s = zqlookStartbody4 + "&" + bodyVal;
                $.setdata(zqlookStartbody4s, 'zqlookStartbody4');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody4s: ${bodyVal}`);
                bodys = zqlookStartbody4s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody4');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody4s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody41,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody4').replace(zqlookStartbody41 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody4');
