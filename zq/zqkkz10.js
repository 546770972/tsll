...
let zqlookStartbody10= $.isNode() ? (process.env.zqlookStartbody10 ? process.env.zqlookStartbody10 : "") : ($.getdata('zqlookStartbody10') ? $.getdata('zqlookStartbody10') : "")
let zqlookStartbody10Arr = []
let zqlookStartbody10s = ""
...
if (zqlookStartbody10) {
    if (zqlookStartbody10.indexOf("&") == -1) {
        zqlookStartbody10Arr.push(zqlookStartbody10)
    } else if (zqlookStartbody10.indexOf("&") > -1) {
        zqlookStartbody10s = zqlookStartbody10.split("&")
    } else if (process.env.zqlookStartbody10 && process.env.zqlookStartbody10.indexOf('&') > -1) {
        zqlookStartbody10Arr = process.env.zqlookStartbody10.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody10 = fs.readFileSync("zqlookStartbody10.txt", "utf8");
    if (zqlookStartbody10 !== `undefined`) {
        zqlookStartbody10s = zqlookStartbody10.split("\n");
    } else {
...
Object.keys(zqlookStartbody10s).forEach((item) => {
    if (zqlookStartbody10s[item] && !zqlookStartbody10s[item].startsWith("#")) {
        zqlookStartbody10Arr.push(zqlookStartbody10s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody10()
     $.done()
 }else{
    console.log(`共${zqlookStartbody10Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody10Arr.length; k++) {

                zqlookStartbody101 = zqlookStartbody10Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody10() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody10) {
            if (zqlookStartbody10.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody10.indexOf(bodyVal) == -1) {
                zqlookStartbody10s = zqlookStartbody10 + "&" + bodyVal;
                $.setdata(zqlookStartbody10s, 'zqlookStartbody10');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody10s: ${bodyVal}`);
                bodys = zqlookStartbody10s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody10');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody10s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody101,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody10').replace(zqlookStartbody101 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody10');
