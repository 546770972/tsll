...
let zqlookStartbody8= $.isNode() ? (process.env.zqlookStartbody8 ? process.env.zqlookStartbody8 : "") : ($.getdata('zqlookStartbody8') ? $.getdata('zqlookStartbody8') : "")
let zqlookStartbody8Arr = []
let zqlookStartbody8s = ""
...
if (zqlookStartbody8) {
    if (zqlookStartbody8.indexOf("&") == -1) {
        zqlookStartbody8Arr.push(zqlookStartbody8)
    } else if (zqlookStartbody8.indexOf("&") > -1) {
        zqlookStartbody8s = zqlookStartbody8.split("&")
    } else if (process.env.zqlookStartbody8 && process.env.zqlookStartbody8.indexOf('&') > -1) {
        zqlookStartbody8Arr = process.env.zqlookStartbody8.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody8 = fs.readFileSync("zqlookStartbody8.txt", "utf8");
    if (zqlookStartbody8 !== `undefined`) {
        zqlookStartbody8s = zqlookStartbody8.split("\n");
    } else {
...
Object.keys(zqlookStartbody8s).forEach((item) => {
    if (zqlookStartbody8s[item] && !zqlookStartbody8s[item].startsWith("#")) {
        zqlookStartbody8Arr.push(zqlookStartbody8s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody8()
     $.done()
 }else{
    console.log(`共${zqlookStartbody8Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody8Arr.length; k++) {

                zqlookStartbody81 = zqlookStartbody8Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody8() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody8) {
            if (zqlookStartbody8.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody8.indexOf(bodyVal) == -1) {
                zqlookStartbody8s = zqlookStartbody8 + "&" + bodyVal;
                $.setdata(zqlookStartbody8s, 'zqlookStartbody8');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody8s: ${bodyVal}`);
                bodys = zqlookStartbody8s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody8');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody8s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody81,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody8').replace(zqlookStartbody81 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody8');
