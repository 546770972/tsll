...
let zq_timebody2= $.isNode() ? (process.env.zq_timebody2 ? process.env.zq_timebody2 : "") : ($.getdata('zq_timebody2') ? $.getdata('zq_timebody2') : "")
let zq_timebody2Arr = []
let zq_timebody2s = ""
let zqwznum
...
if (zq_timebody2) {
    if (zq_timebody2.indexOf("&") == -1) {
        zq_timebody2Arr.push(zq_timebody2)
    } else if (zq_timebody2.indexOf("&") > -1) {
        zq_timebody2s = zq_timebody2.split("&")
    } else if (process.env.zq_timebody2 && process.env.zq_timebody2.indexOf('&') > -1) {
        zq_timebody2Arr = process.env.zq_timebody2.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()) {
    var fs = require("fs");
    zq_timebody2 = fs.readFileSync("zq_timebody2.txt", "utf8");
    if (zq_timebody2 !== `undefined`) {
        zq_timebody2s = zq_timebody2.split("\n");
    } else {
...
Object.keys(zq_timebody2s).forEach((item) => {
    if (zq_timebody2s[item] && !zq_timebody2s[item].startsWith("#")) {
        zq_timebody2Arr.push(zq_timebody2s[item])
    }
})
...
function getzq_timebody2() {
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/user\/stay.json/)) {
          bodyVal=$request.body
            console.log(bodyVal)
        if (zq_timebody2) {
            if (zq_timebody2.indexOf(bodyVal) > -1) {
                $.log("此阅读请求已存在，本次跳过")
            } else if (zq_timebody2.indexOf(bodyVal) == -1) {
                zq_timebody2s = zq_timebody2 + "&" + bodyVal;
                $.setdata(zq_timebody2s,'zq_timebody2');
                $.log(`${$.name}获取阅读: 成功, zq_timebody2s: ${bodyVal}`);
                bodys = zq_timebody2s.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个阅读请求: 成功🎉", ``)
            }
        } else {
            $.setdata($request.body,'zq_timebody2');
            $.log(`${$.name}获取阅读: 成功, zq_timebody2s: ${bodyVal}`);
            $.msg($.name, `获取第一个阅读请求: 成功🎉`, ``)
        }
    }
