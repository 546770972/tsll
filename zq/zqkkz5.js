...
let zqlookStartbody5= $.isNode() ? (process.env.zqlookStartbody5 ? process.env.zqlookStartbody5 : "") : ($.getdata('zqlookStartbody5') ? $.getdata('zqlookStartbody5') : "")
let zqlookStartbody5Arr = []
let zqlookStartbody5s = ""
...
if (zqlookStartbody5) {
    if (zqlookStartbody5.indexOf("&") == -1) {
        zqlookStartbody5Arr.push(zqlookStartbody5)
    } else if (zqlookStartbody5.indexOf("&") > -1) {
        zqlookStartbody5s = zqlookStartbody5.split("&")
    } else if (process.env.zqlookStartbody5 && process.env.zqlookStartbody5.indexOf('&') > -1) {
        zqlookStartbody5Arr = process.env.zqlookStartbody5.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody5 = fs.readFileSync("zqlookStartbody5.txt", "utf8");
    if (zqlookStartbody5 !== `undefined`) {
        zqlookStartbody5s = zqlookStartbody5.split("\n");
    } else {
...
Object.keys(zqlookStartbody5s).forEach((item) => {
    if (zqlookStartbody5s[item] && !zqlookStartbody5s[item].startsWith("#")) {
        zqlookStartbody5Arr.push(zqlookStartbody5s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody5()
     $.done()
 }else{
    console.log(`共${zqlookStartbody5Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody5Arr.length; k++) {

                zqlookStartbody51 = zqlookStartbody5Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody5() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody5) {
            if (zqlookStartbody5.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody5.indexOf(bodyVal) == -1) {
                zqlookStartbody5s = zqlookStartbody5 + "&" + bodyVal;
                $.setdata(zqlookStartbody5s, 'zqlookStartbody5');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody5s: ${bodyVal}`);
                bodys = zqlookStartbody5s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody5');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody5s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody51,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody5').replace(zqlookStartbody51 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody5');
