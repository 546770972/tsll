...
let zqlookStartbody19= $.isNode() ? (process.env.zqlookStartbody19 ? process.env.zqlookStartbody19 : "") : ($.getdata('zqlookStartbody19') ? $.getdata('zqlookStartbody19') : "")
let zqlookStartbody19Arr = []
let zqlookStartbody19s = ""
...
if (zqlookStartbody19) {
    if (zqlookStartbody19.indexOf("&") == -1) {
        zqlookStartbody19Arr.push(zqlookStartbody19)
    } else if (zqlookStartbody19.indexOf("&") > -1) {
        zqlookStartbody19s = zqlookStartbody19.split("&")
    } else if (process.env.zqlookStartbody19 && process.env.zqlookStartbody19.indexOf('&') > -1) {
        zqlookStartbody19Arr = process.env.zqlookStartbody19.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody19 = fs.readFileSync("zqlookStartbody19.txt", "utf8");
    if (zqlookStartbody19 !== `undefined`) {
        zqlookStartbody19s = zqlookStartbody19.split("\n");
    } else {
...
Object.keys(zqlookStartbody19s).forEach((item) => {
    if (zqlookStartbody19s[item] && !zqlookStartbody19s[item].startsWith("#")) {
        zqlookStartbody19Arr.push(zqlookStartbody19s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody19()
     $.done()
 }else{
    console.log(`共${zqlookStartbody19Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody19Arr.length; k++) {

                zqlookStartbody191 = zqlookStartbody19Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody19() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody19) {
            if (zqlookStartbody19.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody19.indexOf(bodyVal) == -1) {
                zqlookStartbody19s = zqlookStartbody19 + "&" + bodyVal;
                $.setdata(zqlookStartbody19s, 'zqlookStartbody19');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody19s: ${bodyVal}`);
                bodys = zqlookStartbody19s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody19');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody19s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody191,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody19').replace(zqlookStartbody191 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody19');
