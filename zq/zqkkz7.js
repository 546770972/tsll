...
let zqlookStartbody7= $.isNode() ? (process.env.zqlookStartbody7 ? process.env.zqlookStartbody7 : "") : ($.getdata('zqlookStartbody7') ? $.getdata('zqlookStartbody7') : "")
let zqlookStartbody7Arr = []
let zqlookStartbody7s = ""
...
if (zqlookStartbody7) {
    if (zqlookStartbody7.indexOf("&") == -1) {
        zqlookStartbody7Arr.push(zqlookStartbody7)
    } else if (zqlookStartbody7.indexOf("&") > -1) {
        zqlookStartbody7s = zqlookStartbody7.split("&")
    } else if (process.env.zqlookStartbody7 && process.env.zqlookStartbody7.indexOf('&') > -1) {
        zqlookStartbody7Arr = process.env.zqlookStartbody7.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody7 = fs.readFileSync("zqlookStartbody7.txt", "utf8");
    if (zqlookStartbody7 !== `undefined`) {
        zqlookStartbody7s = zqlookStartbody7.split("\n");
    } else {
...
Object.keys(zqlookStartbody7s).forEach((item) => {
    if (zqlookStartbody7s[item] && !zqlookStartbody7s[item].startsWith("#")) {
        zqlookStartbody7Arr.push(zqlookStartbody7s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody7()
     $.done()
 }else{
    console.log(`共${zqlookStartbody7Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody7Arr.length; k++) {

                zqlookStartbody71 = zqlookStartbody7Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody7() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody7) {
            if (zqlookStartbody7.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody7.indexOf(bodyVal) == -1) {
                zqlookStartbody7s = zqlookStartbody7 + "&" + bodyVal;
                $.setdata(zqlookStartbody7s, 'zqlookStartbody7');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody7s: ${bodyVal}`);
                bodys = zqlookStartbody7s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody7');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody7s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody71,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody7').replace(zqlookStartbody71 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody7');
