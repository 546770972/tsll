...
let zqlookStartbody17= $.isNode() ? (process.env.zqlookStartbody17 ? process.env.zqlookStartbody17 : "") : ($.getdata('zqlookStartbody17') ? $.getdata('zqlookStartbody17') : "")
let zqlookStartbody17Arr = []
let zqlookStartbody17s = ""
...
if (zqlookStartbody17) {
    if (zqlookStartbody17.indexOf("&") == -1) {
        zqlookStartbody17Arr.push(zqlookStartbody17)
    } else if (zqlookStartbody17.indexOf("&") > -1) {
        zqlookStartbody17s = zqlookStartbody17.split("&")
    } else if (process.env.zqlookStartbody17 && process.env.zqlookStartbody17.indexOf('&') > -1) {
        zqlookStartbody17Arr = process.env.zqlookStartbody17.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody17 = fs.readFileSync("zqlookStartbody17.txt", "utf8");
    if (zqlookStartbody17 !== `undefined`) {
        zqlookStartbody17s = zqlookStartbody17.split("\n");
    } else {
...
Object.keys(zqlookStartbody17s).forEach((item) => {
    if (zqlookStartbody17s[item] && !zqlookStartbody17s[item].startsWith("#")) {
        zqlookStartbody17Arr.push(zqlookStartbody17s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody17()
     $.done()
 }else{
    console.log(`共${zqlookStartbody17Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody17Arr.length; k++) {

                zqlookStartbody171 = zqlookStartbody17Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody17() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody17) {
            if (zqlookStartbody17.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody17.indexOf(bodyVal) == -1) {
                zqlookStartbody17s = zqlookStartbody17 + "&" + bodyVal;
                $.setdata(zqlookStartbody17s, 'zqlookStartbody17');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody17s: ${bodyVal}`);
                bodys = zqlookStartbody17s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody17');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody17s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody171,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody17').replace(zqlookStartbody171 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody17');
