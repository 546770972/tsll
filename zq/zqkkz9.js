...
let zqlookStartbody9= $.isNode() ? (process.env.zqlookStartbody9 ? process.env.zqlookStartbody9 : "") : ($.getdata('zqlookStartbody9') ? $.getdata('zqlookStartbody9') : "")
let zqlookStartbody9Arr = []
let zqlookStartbody9s = ""
...
if (zqlookStartbody9) {
    if (zqlookStartbody9.indexOf("&") == -1) {
        zqlookStartbody9Arr.push(zqlookStartbody9)
    } else if (zqlookStartbody9.indexOf("&") > -1) {
        zqlookStartbody9s = zqlookStartbody9.split("&")
    } else if (process.env.zqlookStartbody9 && process.env.zqlookStartbody9.indexOf('&') > -1) {
        zqlookStartbody9Arr = process.env.zqlookStartbody9.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody9 = fs.readFileSync("zqlookStartbody9.txt", "utf8");
    if (zqlookStartbody9 !== `undefined`) {
        zqlookStartbody9s = zqlookStartbody9.split("\n");
    } else {
...
Object.keys(zqlookStartbody9s).forEach((item) => {
    if (zqlookStartbody9s[item] && !zqlookStartbody9s[item].startsWith("#")) {
        zqlookStartbody9Arr.push(zqlookStartbody9s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody9()
     $.done()
 }else{
    console.log(`共${zqlookStartbody9Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody9Arr.length; k++) {

                zqlookStartbody91 = zqlookStartbody9Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody9() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody9) {
            if (zqlookStartbody9.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody9.indexOf(bodyVal) == -1) {
                zqlookStartbody9s = zqlookStartbody9 + "&" + bodyVal;
                $.setdata(zqlookStartbody9s, 'zqlookStartbody9');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody9s: ${bodyVal}`);
                bodys = zqlookStartbody9s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody9');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody9s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody91,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody9').replace(zqlookStartbody91 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody9');
