...
let zqlookStartbody20= $.isNode() ? (process.env.zqlookStartbody20 ? process.env.zqlookStartbody20 : "") : ($.getdata('zqlookStartbody20') ? $.getdata('zqlookStartbody20') : "")
let zqlookStartbody20Arr = []
let zqlookStartbody20s = ""
...
if (zqlookStartbody20) {
    if (zqlookStartbody20.indexOf("&") == -1) {
        zqlookStartbody20Arr.push(zqlookStartbody20)
    } else if (zqlookStartbody20.indexOf("&") > -1) {
        zqlookStartbody20s = zqlookStartbody20.split("&")
    } else if (process.env.zqlookStartbody20 && process.env.zqlookStartbody20.indexOf('&') > -1) {
        zqlookStartbody20Arr = process.env.zqlookStartbody20.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody20 = fs.readFileSync("zqlookStartbody20.txt", "utf8");
    if (zqlookStartbody20 !== `undefined`) {
        zqlookStartbody20s = zqlookStartbody20.split("\n");
    } else {
...
Object.keys(zqlookStartbody20s).forEach((item) => {
    if (zqlookStartbody20s[item] && !zqlookStartbody20s[item].startsWith("#")) {
        zqlookStartbody20Arr.push(zqlookStartbody20s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody20()
     $.done()
 }else{
    console.log(`共${zqlookStartbody20Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody20Arr.length; k++) {

                zqlookStartbody201 = zqlookStartbody20Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody20() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody20) {
            if (zqlookStartbody20.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody20.indexOf(bodyVal) == -1) {
                zqlookStartbody20s = zqlookStartbody20 + "&" + bodyVal;
                $.setdata(zqlookStartbody20s, 'zqlookStartbody20');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody20s: ${bodyVal}`);
                bodys = zqlookStartbody20s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody20');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody20s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody201,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody20').replace(zqlookStartbody201 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody20');
