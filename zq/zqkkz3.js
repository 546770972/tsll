...
let zqlookStartbody3= $.isNode() ? (process.env.zqlookStartbody3 ? process.env.zqlookStartbody3 : "") : ($.getdata('zqlookStartbody3') ? $.getdata('zqlookStartbody3') : "")
let zqlookStartbody3Arr = []
let zqlookStartbody3s = ""
...
if (zqlookStartbody3) {
    if (zqlookStartbody3.indexOf("&") == -1) {
        zqlookStartbody3Arr.push(zqlookStartbody3)
    } else if (zqlookStartbody3.indexOf("&") > -1) {
        zqlookStartbody3s = zqlookStartbody3.split("&")
    } else if (process.env.zqlookStartbody3 && process.env.zqlookStartbody3.indexOf('&') > -1) {
        zqlookStartbody3Arr = process.env.zqlookStartbody3.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody3 = fs.readFileSync("zqlookStartbody3.txt", "utf8");
    if (zqlookStartbody3 !== `undefined`) {
        zqlookStartbody3s = zqlookStartbody3.split("\n");
    } else {
...
Object.keys(zqlookStartbody3s).forEach((item) => {
    if (zqlookStartbody3s[item] && !zqlookStartbody3s[item].startsWith("#")) {
        zqlookStartbody3Arr.push(zqlookStartbody3s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody3()
     $.done()
 }else{
    console.log(`共${zqlookStartbody3Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody3Arr.length; k++) {

                zqlookStartbody31 = zqlookStartbody3Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody3() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody3) {
            if (zqlookStartbody3.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody3.indexOf(bodyVal) == -1) {
                zqlookStartbody3s = zqlookStartbody3 + "&" + bodyVal;
                $.setdata(zqlookStartbody3s, 'zqlookStartbody3');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody3s: ${bodyVal}`);
                bodys = zqlookStartbody3s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody3');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody3s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody31,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody3').replace(zqlookStartbody31 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody3');
