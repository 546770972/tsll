...
let zqlookStartbody6= $.isNode() ? (process.env.zqlookStartbody6 ? process.env.zqlookStartbody6 : "") : ($.getdata('zqlookStartbody6') ? $.getdata('zqlookStartbody6') : "")
let zqlookStartbody6Arr = []
let zqlookStartbody6s = ""
...
if (zqlookStartbody6) {
    if (zqlookStartbody6.indexOf("&") == -1) {
        zqlookStartbody6Arr.push(zqlookStartbody6)
    } else if (zqlookStartbody6.indexOf("&") > -1) {
        zqlookStartbody6s = zqlookStartbody6.split("&")
    } else if (process.env.zqlookStartbody6 && process.env.zqlookStartbody6.indexOf('&') > -1) {
        zqlookStartbody6Arr = process.env.zqlookStartbody6.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody6 = fs.readFileSync("zqlookStartbody6.txt", "utf8");
    if (zqlookStartbody6 !== `undefined`) {
        zqlookStartbody6s = zqlookStartbody6.split("\n");
    } else {
...
Object.keys(zqlookStartbody6s).forEach((item) => {
    if (zqlookStartbody6s[item] && !zqlookStartbody6s[item].startsWith("#")) {
        zqlookStartbody6Arr.push(zqlookStartbody6s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody6()
     $.done()
 }else{
    console.log(`共${zqlookStartbody6Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody6Arr.length; k++) {

                zqlookStartbody61 = zqlookStartbody6Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody6() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody6) {
            if (zqlookStartbody6.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody6.indexOf(bodyVal) == -1) {
                zqlookStartbody6s = zqlookStartbody6 + "&" + bodyVal;
                $.setdata(zqlookStartbody6s, 'zqlookStartbody6');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody6s: ${bodyVal}`);
                bodys = zqlookStartbody6s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody6');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody6s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody61,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody6').replace(zqlookStartbody61 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody6');
