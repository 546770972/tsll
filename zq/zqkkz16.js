...
let zqlookStartbody16= $.isNode() ? (process.env.zqlookStartbody16 ? process.env.zqlookStartbody16 : "") : ($.getdata('zqlookStartbody16') ? $.getdata('zqlookStartbody16') : "")
let zqlookStartbody16Arr = []
let zqlookStartbody16s = ""
...
if (zqlookStartbody16) {
    if (zqlookStartbody16.indexOf("&") == -1) {
        zqlookStartbody16Arr.push(zqlookStartbody16)
    } else if (zqlookStartbody16.indexOf("&") > -1) {
        zqlookStartbody16s = zqlookStartbody16.split("&")
    } else if (process.env.zqlookStartbody16 && process.env.zqlookStartbody16.indexOf('&') > -1) {
        zqlookStartbody16Arr = process.env.zqlookStartbody16.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody16 = fs.readFileSync("zqlookStartbody16.txt", "utf8");
    if (zqlookStartbody16 !== `undefined`) {
        zqlookStartbody16s = zqlookStartbody16.split("\n");
    } else {
...
Object.keys(zqlookStartbody16s).forEach((item) => {
    if (zqlookStartbody16s[item] && !zqlookStartbody16s[item].startsWith("#")) {
        zqlookStartbody16Arr.push(zqlookStartbody16s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody16()
     $.done()
 }else{
    console.log(`共${zqlookStartbody16Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody16Arr.length; k++) {

                zqlookStartbody161 = zqlookStartbody16Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody16() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody16) {
            if (zqlookStartbody16.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody16.indexOf(bodyVal) == -1) {
                zqlookStartbody16s = zqlookStartbody16 + "&" + bodyVal;
                $.setdata(zqlookStartbody16s, 'zqlookStartbody16');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody16s: ${bodyVal}`);
                bodys = zqlookStartbody16s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody16');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody16s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody161,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody16').replace(zqlookStartbody161 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody16');
