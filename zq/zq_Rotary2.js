...
!(async () => {
        console.log(`共${zq_cookie2Arr.length}个cookie`)
	        for (let k = 0; k < zq_cookie2Arr.length; k++) {
                ...
                zq_cookie21= cookie_id + '&request_time=' + time1 + '&time=' + time1 +'&'+ bodyVal
                //待处理cookie
                ...
                for(let k = 0 ; k < 100 ; k++){
                    await Rotary(zq_cookie21,cookie_id,time)
                    await $.wait(6000);
                    console.log("\n\n")
                }
                for (let k = 1 ; k < 5 ; k++){
                    id = k.toString()
                    await openbox(zq_cookie21,cookie_id,time,id)
                    await $.wait(15000)
                }
                console.log("\n\n")
            }
    ...
    .finally(() => $.done())
//抽奖
function Rotary(zq_cookie21,cookie_id,time) {
    return new Promise((resolve, reject) => {
        let url = {
            url : 'https://kandian.wkandian.com/WebApi/RotaryTable/turnRotary?_='+zq_cookie21,
            headers : {'Host': 'kandian.wkandian.com',
                'Referer':'https://kandian.wkandian.com/html/rotaryTable/index.html?'+zq_cookie21
            },
            body:cookie_id,
        }
...
function openbox(zq_cookie21,cookie_id,time,k,timeout = 0) {
